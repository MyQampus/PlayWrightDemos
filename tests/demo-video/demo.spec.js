...
test("create period in section", async ({ page }) => {
  ...
  // Select Subject
  await page
    .locator("div > div > .min-w-1 > section > div > div")
    .first()
    .click();
  ...
  */
  ...
  await page
    .locator(
      ".mt-3 > div > div:nth-child(2) > div:nth-child(2) > div > .relative > div > div"
    )
    .first()
    .click();
...
test("received student fee", async ({ page }) => {
  ...
  await page
    .getByRole("row", { name: "AR ali raza" })
    .getByRole("img")
    .first()
    .click();
  await page.waitForTimeout(1000);
...
test.describe("Exam module test cases ", () => {
  ...
  test(" add exam schedule  published", async ({ page }) => {
    ...
    await page.goto(`${baseUrl}/exam-planner/exams`);
    await page
      .getByRole("row", { name: "mid term phy Assignment Type" })
      .getByRole("img")
      .click();
  ...
  test("add  section result", async ({ page }) => {
    ...
    await page
      .locator(
        "div:nth-child(4) > div:nth-child(2) > div > div > div > div > .min-w-1 > section > div > div > .w-full"
      )
      .click();
