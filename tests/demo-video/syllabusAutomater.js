...
export async function AddSyllabusAutomater(page,) {
  ...
  await fillInputAfterLabel(page, "Course Name *", "Common Core Standards", "text");
  await fillInputAfterLabel(page, "Course Description", "A course based on US Common Core standards, covering all key subjects.", "text");
...
export async function AddSubjectSyllabusTemplate(page) {
  ...
  await fillInputAfterLabel(page, "Course Provider *", "Common Core Standards", "singleSelect");
  ...
  await fillInputAfterLabel(page, "Chapter Title *", "Linear Equations", "text");
  await fillInputAfterLabel(page, "Description", "Introduction to solving linear equations.", "text");
  await fillInputAfterLabel(page, "Chapter Number *", "1", "text");
  await page.getByRole("button", { name: "Create" }).click();
  await page.getByRole("button", { name: "Add Topics" }).click();
  await fillInputAfterLabel(page, "Chapter *", "Linear Equations", "singleSelect");
  await fillInputAfterLabel(page, "Title *", "Solving Single Variable Equations", "text");
  await fillInputAfterLabel(page, "Description", "Learn how to solve for x in linear equations.", "text");
  ...
  await fillInputAfterLabel(page, "Chapter *", "Linear Equations", "singleSelect");
  await fillInputAfterLabel(page, "Topic *", "Solving Single Variable Equations", "singleSelect");
  await fillInputAfterLabel(page, "Title *", "Basic Equation Solving", "text");
  await fillInputAfterLabel(page, "Description", "Understand basic steps in solving simple algebraic equations.", "text");
